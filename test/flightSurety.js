...
contract('Flight Surety Tests', async (accounts) => {
  ...
  before('setup contract', async () => {
    ...
    INITIAL_FOUND = await config.flightSuretyData.AIRLINE_MIN_FUNDS.call();
    MAX_INSURANCE_POLICY = await config.flightSuretyData.MAX_INSURANCE_POLICY.call();
    await config.flightSuretyApp.sendTransaction({from: config.firstAirline, value: INITIAL_FOUND});
  ...
  it('(airline) cannot register an Airline using registerAirline() if it is not funded', async () => {
    ...
    assert.equal(result, true, "Airline should be registred");
    result = await config.flightSuretyData.isAirlineFunded.call(newAirline);
    assert.equal(result, false, "Airline is not funded");
    ...
    assert.equal(result, false, "Not be able to register nwe airline");
  });

  it("First airline is registred when contract is deployed", async ()=> {
    let result = await config.flightSuretyData.isAirlineRegistered.call(config.firstAirline);
    assert.equal(result, true, "First airline registered");
  });

  it("Only existing airline may register a new airline until there are at least four airlines registerd", async () => {
    const account_offset = 4;
    const max_airlines = 2;

    for(let i=0; i<max_airlines; ++i){
        try{
            await config.flightSuretyApp.sendTransaction({from: accounts[i + account_offset], value: INITIAL_FOUND});
  ...
  it("Registration of fifth and subsecuent airlines requires multi-party consensu of 50% of registered airlines", async () => {
    const account_offset = 6;
    const vote_offset = 4;
    ...
    for(let i = 0; i < max_airlines; ++i){
        ...
        for (let k = 0; i < votes_needed; ++k){
            ...
            let result = await config.flightSuretyData.isAirlineRegistered.call(accounts[i+account_offset]);
            assert.equal(result, k=== (votes_needed - 1), "multy-party consensus failed");
        }
    }
  });
  ...
  it("Airline can be registered, but does not participate in contract until it submits funding of 10 ether", async () => {
    ...
    assert.equal(funded, false, "Airline unfunded");
    let pass;
    try{
    ...
    assert.equal(pass, false, "Airline can't participate without funding");
  });
  ...
  it("Register Flight", async () => {
    for(let i = 0; i < 10; ++i){
        ...
        await config.flightSuretyApp.sendTransaction({from:airline, value: INITIAL_FOUND});
        let funded = await config.flightSuretyData.isAirlineFunded.call(airlines);
        assert.equal.apply(funded, true, "Airline should be funded");
  ...
  it("Passengers may pay up to 1 ether for purchasing flight insurance", async () => {
    ...
    for(let i = 0, len = insurance_values.length; i < len; i++){
        let name = "Flight " + i;
        ...
        if(overpaid_amount > 0) {
            ...
            truffleAssert.eventEmitted(newTx, 'InsureeCredited', null, 'Insuree should not gain any credit');
        }
    }
  ...
  it("If flight is delayed due to airline fault, passenger receives credit of 1.5X the amount they paid", async () => {
    ...
    let insurance_value = new BN(web3.utilstoWei('10', 'ether'));
    let expected_payout;
    if(insurance_value.gt(MAX_INSURANCE_POLICY)){
    ...
    tx = await config.flightSuretyApp.registerOracle({from: accounts[a], value: fee});
    ...
    for(let a = 1; a < TEST_ORACLES_COUNT; a++){
        ...
        for(let idx = 0; idx < 3; idx++){
            ...
            if(success_responses >=3 ){
                ...
                truffleAssert.eventEmitted(tx_data, 'InsureeCredited', null, 'InsureeCredited was no emitted');
  ...
  it("Passenger can withdraw any funds owed to them as a result of receiving credit for insurance payout", async () => {
    let customer = accounts[55];
    let balance = web3.utils.frowWei(await web3.eth.getBalance(customer), 'ether');
